...
export class TosService {
  ...
  private async withXtTokenRetry<T>(
    ...
    const runWithCurrentToken = async () => {
    ...
    try {
      ...
      console.warn(
        `[TosService] ${actionName} 检测到 XT Token 可能失效，尝试刷新会话后重试: ${this.getErrorMessage(error)}`,
      );
  ...
  async initTosClient(
    ...
    // 获取 TOS 临时凭证
    const credentials = await this.withXtTokenRetry(
      configService,
      "获取 TOS 临时凭证",
      async (xtToken) => await this.getTosCredentials(xtToken),
    );
  ...
  private async getTosCredentials(xtToken: string): Promise<TosCredentials> {
    ...
    const response = await axios.get(
      "https://splay-admin.lnkaishi.cn/material/getTosKey",
      {
        headers: {
          token: xtToken,
        },
    ...
    // 兼容不同的返回格式
    const resultData =
      response.data.data?.Result || response.data.Result || response.data;
  ...
  async uploadFile(
    ...
    for (let attempt = 1; attempt <= this.MAX_UPLOAD_RETRY; attempt++) {
      ...
      try {
        ...
        if (attempt === 1) {
          ...
          console.log(
            `[TosService] 重试上传: ${fileName} (${attempt}/${this.MAX_UPLOAD_RETRY})`,
          );
        }
        ...
        timeoutId = setTimeout(() => {
          ...
          if (currentAttemptValid && this.uploadCancelTokens.has(fileName)) {
            ...
            cancelTokenSource.cancel(
              `上传超时（超过 ${this.UPLOAD_TIMEOUT_MS / 60000} 分钟）`,
            );
          }
        ...
        console.log(
          `[TosService] 上传成功: ${fileName} -> ${url}${attempt > 1 ? ` (第 ${attempt} 次尝试)` : ""}`,
        );
        ...
        if (
          !currentAttemptValid ||
          errorMessage === "用户取消上传" ||
          errorMessage === "cancel uploadFile"
        ) {
        ...
        if (isTimeout) {
          console.warn(
            `[TosService] 上传超时: ${fileName} (${attempt}/${this.MAX_UPLOAD_RETRY})`,
          );
        } else {
          console.error(
            `[TosService] 上传失败: ${fileName} (${attempt}/${this.MAX_UPLOAD_RETRY})`,
            lastError,
          );
        }
        ...
        if (attempt < this.MAX_UPLOAD_RETRY) {
          ...
          console.log(
            `[TosService] ${retryReason}，等待 ${this.RETRY_DELAY_MS / 1000} 秒后重试...`,
          );
    ...
    // 所有重试都失败
    console.error(
      `[TosService] 上传最终失败: ${fileName}，已重试 ${this.MAX_UPLOAD_RETRY} 次`,
    );
  ...
  async uploadBatch(
    ...
    const processQueue = async (): Promise<void> => {
      while (
        ...
        this.uploadingCount < this.maxConcurrentUploads
      ) {
  ...
  getUploadDuration(): string {
    ...
    const duration = ((performance.now() - this.totalStartTime) / 1000).toFixed(
      2,
    );
    return duration;
  }
