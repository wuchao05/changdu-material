...
export class ApiService {
    ...
    if (!response.ok) {
      const message =
        typeof data === "object" && data
          ? (data as { message?: string; msg?: string; error?: string })
              .message ||
  ...
  async getPendingUploadDramasByDate(
    ...
    console.log(
      `[ApiService] 查询日期: ${dateObj.toLocaleDateString()}, 时间戳: ${exactDateTimestamp}`,
    );
    ...
    console.log(
      `[ApiService] 飞书返回: code=${response.data.code}, items=${response.data.data?.items?.length || 0}`,
    );
    ...
    if (response.data.data?.items) {
      ...
      console.log(
        `[ApiService] 日期筛选后剩余: ${filteredItems.length} 条记录`,
      );
    }
  ...
  async updateFeishuRecordStatus(
    ...
    try {
      ...
      console.error(
        `[ApiService] 更新飞书状态异常: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
  ...
  async getRemotePendingBuildDramas(
    ...
    const response = await fetch(
      `${REMOTE_API_BASE_URL}/feishu/bitable/drama-status/pending-build`,
      {
        method: "POST",
        headers: {
  ...
  async startRemoteDailyBuildScheduler(
    ...
    const response = await fetch(
      `${REMOTE_API_BASE_URL}/build-workflow/scheduler/start`,
      {
        method: "POST",
        headers: {
          ...configService.getWebSessionHeaders({
  ...
  async stopRemoteDailyBuildScheduler(
    ...
    const response = await fetch(
      `${REMOTE_API_BASE_URL}/build-workflow/scheduler/stop`,
      {
        method: "POST",
        headers: {
          ...configService.getWebSessionHeaders({
  ...
  async triggerRemoteDailyBuildScheduler(
    ...
    const response = await fetch(
      `${REMOTE_API_BASE_URL}/build-workflow/scheduler/trigger`,
      {
        method: "POST",
        headers: {
          ...configService.getWebSessionHeaders({
  ...
  private async getFeishuToken(): Promise<string> {
    // 检查缓存
    if (
      this.feishuTokenCache.token &&
      Date.now() < this.feishuTokenCache.expireTime
    ) {
    ...
    this.feishuTokenCache.expireTime =
  ...
  async changduRequest(
    ...
    console.log("[ApiService] 请求头:", {
      ...requestHeaders,
      Cookie: requestConfig.cookie
        ? `${requestConfig.cookie.substring(0, 50)}...`
        : "no cookie",
    });
  ...
  async submitToMaterialLibrary(
    ...
    console.log(
      `[ApiService] 素材库请求 URL: https://splay-admin.lnkaishi.cn/material/add?team_id=500039`,
    );
    console.log(
      `[ApiService] 提交素材列表:`,
