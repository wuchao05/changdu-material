 ...
 * 进度管理器 - 用于巨量上传断点续传
 */
export class JuliangProgressManager {
  private progressFile: string;
  private progressMap: Map<string, UploadProgress> = new Map();

  constructor() {
    // 使用 userData 目录存储进度
    const progressDir = path.join(app.getPath("userData"), "juliang-progress");
  ...
  private loadProgress(): void {
    try {
      if (fs.existsSync(this.progressFile)) {
        ...
        console.log(`[JuliangProgress] 已加载 ${this.progressMap.size} 条上传进度记录`);
      }
    } catch (error) {
      console.error(
        `[JuliangProgress] 加载进度文件失败: ${
  ...
  public updateProgress(
    ...
    console.log(
      `[JuliangProgress] 已更新进度: ${drama} - 完成 ${completedBatches}/${totalBatches} 批`
    );
  }
  ...
  public clearProgress(recordId: string, drama: string): void {
    if (this.progressMap.has(recordId)) {
      ...
      this.saveProgress();
      console.log(`[JuliangProgress] 已清除上传进度: ${drama}`);
    }
  }
  ...
  public clearAllProgress(): void {
    this.progressMap.clear();
    this.saveProgress();
    console.log("[JuliangProgress] 已清除所有上传进度");
  }
